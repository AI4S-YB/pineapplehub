...
综合评分最高的候选 $\mathcal{C}^* = \arg\max_i \mathcal{S}_i$ 被选定为果皮 ROI **目标**。

*物理依据*：菠萝果皮布满隆起的果眼，各果眼之间由深色缝隙分隔，产生高 $\bar{g}$。切开的果肉表面光滑，$\bar{g} \approx 0$。硬币虽边缘对比强烈，但面积小，$\sqrt{A}$ 项形成有效的尺寸惩罚。

#### 2.3 低阈值定界

第 1.2 节所得 Otsu 轮廓可能是**碎片化**的：深色果眼间缝隙的亮度常低于 Otsu 阈值，将果实轮廓分裂为多个不连通的白色区域。纹理评分可以正确**识别**哪个候选是果皮侧，但中选候选的轮廓可能只覆盖果实的一部分。

为获取用于 ROI 计算的**完整**果实轮廓，对平滑灰度图像 $I_{smooth}$ 以低固定阈值 $\tau_{low} = 25$ 做全局二值化：

$$B_{low} = \mathbf{1}[I_{smooth} > \tau_{low}]$$

在此阈值下，果眼间暗隙（平滑后亮度 ≈ 30–50）仍高于阈值，使果实保持为单一连通域。而背景（亮度 ≈ 0–15）低于阈值，天然分隔不同物体，无需形态学闭运算。

在 $B_{low}$ 上提取轮廓。选取 AABB 包含 $\mathcal{C}^*$ 质心且与 $\mathcal{C}^*$ 的 AABB 重叠面积最大的轮廓作为定界轮廓 $\mathcal{C}_{bound}$。其 `min_area_rect` 即为最终 ROI 参数。

> 该**两阶段**设计将*识别*（Otsu + 纹理评分）和*定界*（低阈值分割）分离。Otsu 轮廓善于区分物体类型但会碎片化；低阈值轮廓提供完整轮廓但不区分物体类型。两阶段互补。
