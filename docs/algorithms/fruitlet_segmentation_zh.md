...
旋转后，菠萝的顶底两极转至图像横向两侧；展开器以 $f = r = H_{roi} \geq W_{roi}$ 施加更强的横向拉伸，消除纵向（极点）方向的透视缩短：
...
1.  **全局 Otsu 阈值化** → 二值掩膜。
2.  **0.25× 缩小**（最近邻），随后进行形态学闭运算（半径 2，$L_\infty$）再进行开运算（半径 3，$L_\infty$），最后**4× 放大**回原始分辨率。多尺度处理在抑制内部噪声的同时保留了果实整体轮廓。
3.  按周长选取**最长轮廓**。
